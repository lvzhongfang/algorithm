...
public class LCS {
    ...
    public static String LCS1 (String s1, String s2) {
        ...
        int m = len2;
        while (dp[n][m] != 0) {
            //来自于上方,与上方相等则说明s1中 n - 1位置的字符与s2中 m - 1位置的字符不相同，最大公共子序列的长度选择了s1的前 n - 1个字符与s2的前 m个字符的最大公共子序列长度，
            // 此时s1中的 n - 1位置的字符不属于公共子序列，无需添加
            if (dp[n][m] == dp[n - 1][m]) {
                n--;
                ...
                //来自于左上方向
                //说明s1的前n个字符与s2的前m个字符的最大公共子序列的长度是由dp[i - 1][j - 1] + 1计算得到的，s1中 n - 1位置的字符是最大公共子序列的内容，添加到序列中
                n--;
                m--;
